...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        Object objM2273constructorimpl;
        boolean z;
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        objM2273constructorimpl = SegmentOrClosed.m2273constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM2273constructorimpl = SegmentOrClosed.m2273constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (SegmentOrClosed.m2278isClosedimpl(objM2273constructorimpl)) {
                break;
            }
            Segment segmentM2276getSegmentimpl = SegmentOrClosed.m2276getSegmentimpl(objM2273constructorimpl);
            while (true) {
                Segment segment = (Segment) this.tail;
                if (segment.getId() >= segmentM2276getSegmentimpl.getId()) {
                    break;
                }
                if (!segmentM2276getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z = false;
                    break;
                }
                if (LifecycleKt$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, segmentM2276getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (segmentM2276getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                    segmentM2276getSegmentimpl.remove();
                }
            }
            z = true;
        } while (!z);
        SemaphoreSegment semaphoreSegment3 = (SemaphoreSegment) SegmentOrClosed.m2276getSegmentimpl(objM2273constructorimpl);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        do {
            ...
            while (true) {
                ...
                if (LifecycleKt$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment, segmentM2276getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (segmentM2276getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
